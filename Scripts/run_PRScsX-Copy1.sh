...
process_chromosome() {
    ...
    output_dir="/pollard/home/yorkz/york/BMI206/Simulated_pop/Split/Overfitting_test/Plink/PRScsX/S2/"
    # Change this to Plink score output dir
    output_score_dir="/pollard/home/yorkz/york/BMI206/Simulated_pop/Split/Overfitting_test/Plink/Scores/S2/"
    ...
    output_prefix="${target_dir}/AFR_chr${chr}.${col}.target"
    python ${PRScsx_path}PRScsx.py --ref_dir=${ref_dir} --bim_prefix=$output_prefix --sst_file=${sst_dir}${pop}_chr${chr}_${size}.${col}.discovery.formatted.assoc.linear --n_gwas=$((ng - 1)) --pop=${pop} --out_dir=${output_dir} --out_name=AFR_target_${pop}_discovery_chr${chr}_${size}_${col} --chrom=${chr}
    ...
    ${plink_path}plink --bfile ${target_dir}/AFR_chr${chr}.${col}.target --score ${output_dir}/AFR_target_${pop}_discovery_chr${chr}_${size}_${col}_${pop}_pst_eff_a1_b0.5_phiauto_chr${chr}.txt 2 5 6 sum --allow-no-sex --out ${output_score_dir}${pop}_chr${chr}_${size}_${col}_score
    
    echo "Processing $pop discovery ${size} AFR target chromosome $chr ${col}"
}
export -f process_chromosome
# This is an intermediate file, change the file path to your working dir, keep the file name.
cmd_file="/pollard/home/yorkz/york/BMI206/Simulated_pop/Split/Overfitting_test/Plink/Scores/S2/process_commands.txt"
> ${cmd_file}
